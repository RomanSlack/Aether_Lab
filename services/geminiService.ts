...
const paramsSchema: Schema = {
  ...
  properties: {
    ...
    scale: { type: Type.NUMBER, description: "Zoom scale of noise (0.001 to 0.01). Lower is more zoomed in/smooth, Higher is more detailed/noisy." },
  ...
  required: ["baseColor", "secondaryColor", "accentColor", "scale", "distortion", "contrast", "brightness"]
};
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: "You are an expert generative artist and colorist. You translate abstract moods and descriptions into precise mathematical parameters for a domain-warping noise shader. Scale should usually be between 0.002 and 0.008. Distortion usually between 2.0 and 8.0. Colors should form a cohesive, high-end palette."
      }
    });
